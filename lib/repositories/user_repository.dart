...
class UserRepository {
  ...
  Future<AppUser?> getUser(String id) => _db.getUserById(id);
  ...
  Future<void> updateProfile({
    ...
    final u = await _db.getUserById(userId);
    if (u == null) return;
  ...
  Future<void> switchAppRole({
    ...
    await _db.upsertUser(u.copyWith(role: newRole));
  ...
  Future<void> adminSetRole({
    ...
    final u = await _db.getUserById(targetUserId);
    if (u == null) throw StateError('user_not_found');
    await _db.upsertUser(
      u.copyWith(
        role: role,
  ...
  Future<void> setBlocked(String userId, bool blocked) async {
    final u = await _db.getUserById(userId);
    if (u == null) throw StateError('user_not_found');
    await _db.upsertUser(u.copyWith(blocked: blocked));
  }
  ...
  Future<void> applyOrderFeedbackImpact({
    ...
    final u = await _db.getUserById(toUserId);
    if (u == null) return;
