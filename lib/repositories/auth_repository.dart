...
class AuthRepository {
  ...
  Future<AppUser> submitPhone(String rawPhone) async {
    ...
    final existing = await _db.getUserByPhone(normalized);
    if (existing != null) {
  ...
  Future<AppUser> verifySms({
    ...
    var user = await _db.getUserByPhone(normalized);
    if (user == null) {
      user = AppUser(
        id: _uuid.v4(),
        phone: normalized,
  ...
  Future<AppUser> completeProfileSetup({
    ...
    await _db.upsertUser(next);
    final saved = await _db.getUserById(id);
    if (saved == null) {
      throw StateError('no_user');
    }
  ...
  Future<void> saveProfileTransportTypes(
    ...
    final user = await _db.getUserById(userId);
    if (user != null) {
      await _db.upsertUser(user.copyWith(courierTransportTypes: norm));
    }
  }
