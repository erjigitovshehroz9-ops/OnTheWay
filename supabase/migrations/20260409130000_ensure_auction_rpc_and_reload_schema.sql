-- Agar 20260404100000_auction_rpc.sql remote’da to‘liq ishlamagan bo‘lsa yoki PostgREST cache
-- `finalize_expired_auctions` / `start_or_step_auction` ni ko‘rmasa — ushbu fayl qayta yaratadi va
-- schema cache’ni yangilaydi. Order/realtime oqimiga tegmaydi (faqat auksion RPC + kerakli ustunlar).
...
ALTER TABLE public.bids ADD COLUMN IF NOT EXISTS price_before_cents bigint;
ALTER TABLE public.bids ADD COLUMN IF NOT EXISTS price_after_cents bigint;
...
BEGIN
  ...
  SET
    status = 'assigned',
    winner_courier_id = trim(both from o.leading_courier_id),
    final_price_cents = COALESCE(o.current_price_cents, o.start_price_cents, o.price)::bigint,
    winner_selected_at = now(),
    auction_ends_at = NULL,
    auction_expires_at = NULL,
    leading_courier_id = trim(both from o.leading_courier_id)
  WHERE o.status = 'auction_live'
    AND o.leading_courier_id IS NOT NULL
    AND trim(both from o.leading_courier_id) <> ''
    AND COALESCE(o.auction_expires_at, o.auction_ends_at) IS NOT NULL
    AND COALESCE(o.auction_expires_at, o.auction_ends_at) < now();
  GET DIAGNOSTICS v_winners = ROW_COUNT;

  UPDATE public.orders o
  SET
    status = 'posted',
    winner_courier_id = NULL,
    final_price_cents = NULL,
    winner_selected_at = NULL,
    auction_ends_at = NULL,
    auction_expires_at = NULL,
    leading_courier_id = NULL,
    auction_last_courier_id = NULL,
    auction_bid_count = 0,
    auction_step = 0,
  ...
  WHERE o.status = 'auction_live'
    AND (o.leading_courier_id IS NULL OR trim(both from o.leading_courier_id) = '')
    AND COALESCE(o.auction_expires_at, o.auction_ends_at) IS NOT NULL
    AND COALESCE(o.auction_expires_at, o.auction_ends_at) < now();
  GET DIAGNOSTICS v_reopen = ROW_COUNT;

  RETURN v_winners + v_reopen;
END;
$$;

REVOKE ALL ON FUNCTION public.start_or_step_auction(text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.finalize_expired_auctions() FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.start_or_step_auction(text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.finalize_expired_auctions() TO authenticated;

COMMENT ON FUNCTION public.start_or_step_auction(text, text) IS 'Atomik auksion boshlash (posted) yoki qadam (auction_live); auth.uid() = kuryer';
COMMENT ON FUNCTION public.finalize_expired_auctions() IS 'Muddati o‘tgan auction_live buyurtmalarni assigned yoki posted qiladi';

-- PostgREST schema cache (PGRST "function not found in schema cache")
